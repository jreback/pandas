...
from pandas import DataFrame, Panel, Series, isnull, MultiIndex
...
class OLS(StringMixin):
    ...
    def _f_stat_raw(self):
        ...
        return calc_F(R, r, self._beta_raw, self._var_beta_raw,
                      self._nobs, self.df)
    ...
    def f_test(self, hypothesis):
        ...
        result = calc_F(R, r, self._beta_raw, self._var_beta_raw,
                        self._nobs, self.df)
    ...
    def _var_beta_raw(self):
        ...
        if self._nw_lags is None:
            return inv(xx) * (self._rmse_raw ** 2)
        else:
            resid = y - np.dot(x, self._beta_raw)
            m = (x.T * resid).T

            xeps = newey_west(m, self._nw_lags, self._nobs, self._df_raw,
                              self._nw_overlap)
...
"""
        ...
        params = {
            'bannerTop': banner('Summary of Regression Analysis'),
            'bannerCoef': banner('Summary of Estimated Coefficients'),
            'bannerEnd': banner('End of Summary'),
...
# "raw" attributes, calculations
    ...
    def _f_stat_raw(self):
        ...
        def get_result(beta, vcov, n, d):
            return calc_F(R, r, beta, vcov, n, d)
    ...
    def _var_beta_raw(self):
        ...
        for n, i in enumerate(self._valid_indices):
            ...
            if self._nw_lags is None:
                result = inv(xx) * (rmse[n] ** 2)
            else:
                resid = yv - np.dot(xv, beta[n])
                m = (xv.T * resid).T

                xeps = newey_west(m, self._nw_lags, nobs[n], df[n],
                                  self._nw_overlap)
...
def ols(**kwargs):
    ...
    Rolling simple OLS with window of size 10.
    >>> result = ols(y=y, x=x, window_type='rolling', window=10)
    >>> result.beta

    Specify Newey-West lags
    ...
    if window_type is None:
        ...
        window_type = _get_window_type(window_type)

    if window_type != 'full_sample':
        kwargs['window_type'] = _get_window_type(window_type)
